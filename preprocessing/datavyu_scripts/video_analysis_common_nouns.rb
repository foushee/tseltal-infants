...
STIMULI = %w{baby dog chayote water corn tortilla shoe sheep car horse rabbit cow chicken fire soup soda spoon squash}
LEFT = 0
RIGHT = 1
...
csv_path = File.join(DIR, "#{DIR.split('/').last}-overlaps3133duration-trial.csv")
...
=end
    ...
    when 3
      ...
      fail "Invalid number of wordWindow cells: #{window_cells.length} should be 3"
    ...
    looking_offset_ms = [leftlooks.last, rightlooks.last].compact.map(&:offset).max # FINAL LOOK-AWAY
