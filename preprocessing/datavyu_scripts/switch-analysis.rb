...
STIMULI = %w{old_man old_woman young_man young_woman baby dog chayote water corn tortilla shoe sheep car horse rabbit cow chicken fire soup soda spoon squash}
LEFT = 0
RIGHT = 1
...
csv_path = File.join(DIR, "#{DIR.split('/').last}-switch-analysis.csv")
...
files.each do |file|
  ...
  begin
    ...
    fail "Missing first coder cell" unless coder_cell
...
=end
    ...
    fields = [['subject_id', subject_id],
              ...
              ['time_unit', 'ms'],
              ['trial_dur_s', range_duration([trial_onset, trial_offset]) / 1000.0], ['trial_onset_ms', trial_onset],
              ['looking_onset_ms', looking_onset_ms],
              ['sp_onset_ms', sp_cell && sp_cell.onset],
              ['target_onset_ms', target_cell.onset],
              ['pre_onset_ms', pre_onset], ['pre_offset_ms', pre_offset],
              ['pre_dur_ms', range_duration([pre_onset, pre_offset])],
              ['post1_onset_ms', post1_onset], ['post1_offset_ms', post1_offset],
              ['post1_dur_ms', range_duration([post1_onset, post1_offset])]]
              #['post2_onset_ms', post2_onset], ['post2_offset_ms', post2_offset],
              #['post2_dur_ms', range_duration([post2_onset, post2_offset])]]

    labeled_ranges = []
    labeled_ranges << ['pre', [pre_onset, pre_offset]]
    labeled_ranges << ['origin', [target_cell.onset-1, target_cell.onset]]
    labeled_ranges << ['target', [target_cell.onset+1, post2_offset]]
    #labeled_ranges << ['post', [post1_onset, post2_offset]]
    labeled_ranges << ['post1', [post1_onset, post1_offset]]
    ...
    labeled_ranges.each do |(label, window_range)|
      ...
      if window_range
        ...
        first_look[LEFT] = (overlaps[LEFT].map).first #overlaps.flatten.min
        first_look[RIGHT] = (overlaps[RIGHT].map).first
        #first_look_duration[LEFT] = range_duration(first_look[LEFT])
        #first_look_duration[RIGHT] = range_duration(first_look[RIGHT])
      ...
      fields << ["#{label}_target_first_new_look", first_look[target_index]]
      fields << ["#{label}_nontarget_first_new_look", first_look[1 - target_index]]
      #fields << ["#{label}_target_first_new_look_duration", first_look_duration[target_index]]
      #fields << ["#{label}_nontarget_first_new_look_dration", first_look_duration[1 - target_index]]
    end
